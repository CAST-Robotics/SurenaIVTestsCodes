...
//ros::ServiceServer ImitationService;
ros::ServiceServer GripingService;
ros::Subscriber face_sub;
ros::Subscriber object_sub;
ros::Subscriber sub;
//ros::Subscriber imusub;
//ros::Subscriber sub_basiction;
...
bool move_imitation_active=false;
bool move_griping_active=false;
...
griping grip;
geometry_msgs::PoseArray gripMsg;
...
//    imitationCode=int(msg.poses[0].position.x);
//}
...
void StartPhase(){
    if ( GlobalTime<=DurationOfStartPhase) {
        ...
        //        zStart=OnlineTaskSpace.InitialPelvisHeight+cal.move2pose(OnlineTaskSpace.ReferencePelvisHeight-OnlineTaskSpace.InitialPelvisHeight,GlobalTime,0,DurationOfStartPhase);
        zStart=PelvisCurrentHeight+cal.move2pose(OnlineTaskSpace.ReferencePelvisHeight-PelvisCurrentHeight,GlobalTime,0,DurationOfStartPhase);
...
//                   matrix_view(temp);}
        ...
        if (GlobalTime>=TstartofPitchModify && GlobalTime<=TendofPitchModify){
            PitchModified=cal.move2pose(D_pitch,GlobalTime,TstartofPitchModify,TendofPitchModify);
        }
    }
}
...
void EndPhase(){
    ...
    if (GlobalTime>=(DurationOfStartPhase+OnlineTaskSpace.MotionTime)){// && GlobalTime<=DurationOfendPhase+DurationOfStartPhase+OnlineTaskSpace.MotionTime) {
        ...
        double xStart=0;
        GlobalTime=GlobalTime+OnlineTaskSpace._timeStep;
        zStart=OnlineTaskSpace.ReferencePelvisHeight+cal.move2pose(OnlineTaskSpace.InitialPelvisHeight-OnlineTaskSpace.ReferencePelvisHeight,GlobalTime,DurationOfStartPhase+OnlineTaskSpace.MotionTime,DurationOfStartPhase+OnlineTaskSpace.MotionTime+DurationOfendPhase);
        // zStart=PelvisCurrentHeight+cal.move2pose(OnlineTaskSpace.InitialPelvisHeight-PelvisCurrentHeight,GlobalTime,DurationOfStartPhase+OnlineTaskSpace.MotionTime,DurationOfStartPhase+OnlineTaskSpace.MotionTime+DurationOfendPhase);
...
//               matrix_view(temp);}
        ...
        if (GlobalTime>=TstartofPitchModify && GlobalTime<=TendofPitchModify){
            PitchModified=D_pitch-cal.move2pose(D_pitch,GlobalTime,TstartofPitchModify,TendofPitchModify);
        }
    }
}
...
//    }
    ...
    if (qc_initial_bool||qc_update_bool){
        ...
        if(move_active_offlineData||move_active_offlineTrajectory||move_lower_active||move_upper_activate||move_imitation_active||move_griping_active||qc_update_bool){
...
void publish_data(){
    if(move_upper_activate||move_lower_active||move_active_offlineData||move_active_offlineTrajectory||move_imitation_active||move_griping_active){
        for (int i = 0; i <= 33; ++i) {
...
bool StartWalk(trajectory_generation::walkRequest &req,trajectory_generation::walkResponse &res)
{
    ...
    if(!move_lower_active){
        ...
        case 2:
            ...
            OnlineTaskSpace.RightHipRollModification=2.5;
            OnlineTaskSpace.LeftHipRollModification=2.5;
            OnlineTaskSpace.FirstHipRollModification=2.5;
...
//                alai4.append(QString::number(pelav)+"\n");
                ...
                if (OnlineTaskSpace.globalTime<=OnlineTaskSpace.MotionTime){
                    ...
                    if(!AnkleZAdaptation){AnkleZL=m3;AnkleZR=m7;}
                        ...
                        //cyle z
                            ...
                            // takhlie pye rast
                                ...
                                AnkleZR=OnlineTaskSpace._lenghtOfAnkle+AnkleZ_offsetR-cal.move2pose(AnkleZ_offsetR,local_time_cycle,OnlineTaskSpace.Tc,OnlineTaskSpace.Tc+OnlineTaskSpace.TDs);

                            }

                            // takhlie pye chap
                            if(local_time_cycle<=OnlineTaskSpace.Tc-OnlineTaskSpace.TSS+OnlineTaskSpace.TStartofAnkleAdaptation){
                                AnkleZL=OnlineTaskSpace._lenghtOfAnkle+AnkleZ_offsetL-cal.move2pose(AnkleZ_offsetL,local_time_cycle,0,OnlineTaskSpace.TDs);
                            ...
                            // takhlie pye rast
                                ...
                                AnkleZR=OnlineTaskSpace._lenghtOfAnkle+AnkleZ_offsetR-cal.move2pose(AnkleZ_offsetR,GlobalTime-DurationOfStartPhase,OnlineTaskSpace.MotionTime-OnlineTaskSpace.TE,OnlineTaskSpace.MotionTime-0.66*OnlineTaskSpace.TE);
                    ...
                    coef_y_la=1-cal.move2pose(1-alpha,GlobalTime-DurationOfStartPhase,OnlineTaskSpace.Tx+OnlineTaskSpace.TDs+OnlineTaskSpace.TSS/2+time_margin,OnlineTaskSpace.TStart-time_margin)
                            +cal.move2pose(1-alpha,GlobalTime-DurationOfStartPhase,OnlineTaskSpace.TGait-OnlineTaskSpace.TSS+time_margin,OnlineTaskSpace.TGait-time_margin);

                    coef_y_ra=1-cal.move2pose(1-alpha,GlobalTime-DurationOfStartPhase,OnlineTaskSpace.TStart+OnlineTaskSpace.TDs+time_margin,OnlineTaskSpace.TStart+OnlineTaskSpace.Tc-time_margin)
                            +cal.move2pose(1-alpha,GlobalTime-DurationOfStartPhase,OnlineTaskSpace.TGait+OnlineTaskSpace.TDs+time_margin,OnlineTaskSpace.TGait+OnlineTaskSpace.Tc-time_margin);

                    coef_y_p=1-cal.move2pose(1-alpha,GlobalTime-DurationOfStartPhase,OnlineTaskSpace.TStart+OnlineTaskSpace.TDs/2+time_margin,OnlineTaskSpace.TStart+OnlineTaskSpace.TDs-time_margin)
                            +cal.move2pose(1-alpha,GlobalTime-DurationOfStartPhase,OnlineTaskSpace.TGait+OnlineTaskSpace.TDs/2+time_margin,OnlineTaskSpace.TGait+OnlineTaskSpace.TDs-time_margin);
...
//                               matrix_view(temp);}
                    ...
                    if(!OnlineTaskSpace.side_extra_step_length){
                        right_hand_pitch=-asin(cal.saturate(PoseLFoot(0,0)-PoseRoot(0,0),-.1,1)/.8);
                        left_hand_pitch=-asin(cal.saturate(PoseRFoot(0,0)-PoseRoot(0,0),-.1,1)/.8);
                    }
                    right_hand_pitch+=(cal.move2pose(cal.d2r(-13),GlobalTime-DurationOfStartPhase,0,OnlineTaskSpace.TStart)+cal.move2pose(cal.d2r(13),GlobalTime-DurationOfStartPhase-OnlineTaskSpace.MotionTime,-OnlineTaskSpace.TEnd/2,0));
                    left_hand_pitch+=(cal.move2pose(cal.d2r(-13),GlobalTime-DurationOfStartPhase,0,OnlineTaskSpace.TStart)+cal.move2pose(cal.d2r(13),GlobalTime-DurationOfStartPhase-OnlineTaskSpace.MotionTime,-OnlineTaskSpace.TEnd/2,0));
...
//                               matrix_view(temp);}
                    ...
                    if(sidewalk){
                        ...
                        double side_move_coef_yp=1+(beta_side-1)+cal.move2pose(1-beta_side,PoseRoot(1,0),-OnlineTaskSpace.YpMax,0)+
                                cal.move2pose(alpha_side-1,PoseRoot(1,0),0,OnlineTaskSpace.YpMax);
...
//                alai4.append(QString::number(pelav)+"\n");
            ...
            cntrl[5]=cal.saturate(links[5].JointAngle,-M_PI/5,M_PI/4)+ankle_adaptation_switch*(left_first*teta_motor_R+(!left_first)*teta_motor_L);
            ...
            cntrl[11]=cal.saturate(links[11].JointAngle,-M_PI/5,M_PI/4)+ankle_adaptation_switch*(left_first*teta_motor_L+(!left_first)*teta_motor_R);
...
//                q_lower[i]=0;
//            }
            //q_lower[14]=cal.saturate(outControl,-10,10)*M_PI/180;
            //            q_lower[3]=cal.saturate(outControl,-10,10)*M_PI/180;
            //            q_lower[9]=cal.saturate(outControl,-10,10)*M_PI/180;
            //q_lower[5]=-cal.saturate(outControl,-10,10)*M_PI/180;
            //q_lower[11]=-cal.saturate(outControl,-10,10)*M_PI/180;
...
if(scenario_l=="openFingers"){fingers_mode_l=3;fingers_l=3;}
  ...
  if(scenario_r=="lookAtHorizon"){fingers_mode_r=10;fingers_r=10;}
                ...
                if(scenario_r=="home"||scenario_r=="micDown"){
                    ...
                    for (int i = 0; i < 7; ++i) {
                        q_ra(i)=p_r(i,0)+cal.move2pose(p_r(i,1)-p_r(i,0),time_r,t_h(0),t_h(1));

                    }

                    q_upper[2]=cal.move2zero(q_upper[2],time_r,t_h(1));
                    q_upper[8]=q_upper[2];
                    q_upper[6]=-q_upper[2];
                    ...
                    if(time_r<t_r(1) && time_r>=t_r(0)){
                        ...
                        if(/*scenario_r=="moveFingers"||*/scenario_r=="homeFingers"||scenario_r=="openFingers"){fingers_r=fingers_mode_r;}
                    }
                ...
                if(scenario_l=="home"||scenario_l=="micDown"){
                    ...
                    for (int i = 0; i < 7; ++i) {
                        q_la(i)=p_l(i,0)+cal.move2pose(p_l(i,1)-p_l(i,0),time_l,t_h(0),t_h(1));
                    }
                    ...
                    if(time_l<t_l(1) && time_l>=t_l(0)){
                        ...
                        sai_l=hand_l.sai;
                        phi_l=hand_l.phi;
                        if(/*scenario_l=="moveFingers"||*/scenario_l=="homeFingers"||scenario_l=="openFingers"){fingers_l=fingers_mode_l;}
                ...
                if(scenario_hw=="face"){
                    ...
                    X_face=320;
                    Y_face=200;
                    head_yaw=cal.saturate(head_yaw,-45,45);
                    head_pitch=cal.saturate(head_pitch,-15,20);
//                    (scenario_r=="byebye"||scenario_r=="shakeHands")*t_r(2)
                    ...
                    head_pitch=cal.move2pose(pitchRange,double(count_upper*.005),0,2);
                    if(double(count_upper*.005)>=2){headFinished=true;}

                }
                else if(scenario_hw=="start2"){

                    head_pitch=pitchRange-cal.move2pose(pitchRange,double(count_upper*.005),0,2);
                    if(double(count_upper*.005)>=2){headFinished=true;}
                }
                //look at hands
                    ...
                    head_yaw=cal.move2pose(yawRange-30*M_PI/180,double(count_upper*.005),0,2)+
                            cal.move2pose(-2*(yawRange-30*M_PI/180),double(count_upper*.005),2,6)+
                            cal.move2pose(yawRange-30*M_PI/180,double(count_upper*.005),6,8);
                    q_ra(4)=q0_r(4)+cal.move2pose(-30*M_PI/180,double(count_upper*.005),0,4)+
                            cal.move2pose(30*M_PI/180,double(count_upper*.005),4,8);
                    q_la(4)=q0_l(4)+cal.move2pose(30*M_PI/180,double(count_upper*.005),0,4)+
                            cal.move2pose(-30*M_PI/180,double(count_upper*.005),4,8);
                    ...
                    head_yaw=cal.move2pose(yawRange-20*M_PI/180,double(count_upper*.005),0,2);
                    head_pitch=pitchRange-cal.move2pose(pitchRange,double(count_upper*.005),0,2);
                    WaistYaw=cal.move2pose(WYawRange,double(count_upper*.005),2,4);
                    ...
                    head_yaw=yawRange-20*M_PI/180-cal.move2pose(yawRange-20*M_PI/180,double(count_upper*.005),0,2);

                    WaistYaw=WYawRange-cal.move2pose(WYawRange,double(count_upper*.005),0,2);
                    if(double(count_upper*.005)>=2){headFinished=true;}
                    ...
                    head_pitch=cal.move2pose(pitchRange,double(count_upper*.005),t_l(0)-4,t_l(2));

                    //                    q[2]=cal.move2pose(-.15,double(count*.005),0,1)+
                    //                            cal.move2pose(.3,double(count*.005),3,5)+
                    //                            cal.move2pose(-.15,double(count*.005),7,8);
                    //                    //qDebug()<<double(count*.005)<<'\t'<<q[2];
                    //                    q[8]=q[2];
                    //                    q[6]=-q[2];
                    //                    q[12]=-q[2];

                    //                    q[4]=cal.move2pose(.5,double(count*.005),1,2)+
                    //                            cal.move2pose(-.5,double(count*.005),2,3);
                    //                    q[3]=-q[4]/2;
                    //                    q[5]=-q[4]/2;

                    //                    q[10]=cal.move2pose(.5,double(count*.005),5,6)+
                    //                            cal.move2pose(-.5,double(count*.005),6,7);

                    //                    q[9]=-q[10]/2;
                    //                    q[11]=-q[10]/2;

                    q_upper[2]=cal.move2pose(-.1,double(count_upper*.005),0,1.5)+
                            cal.move2pose(.2,double(count_upper*.005),1.5,4.5)+
                            cal.move2pose(-.1,double(count_upper*.005),4.5,6);
                    //qDebug()<<double(count*.005)<<'\t'<<q[2];
                    q_upper[8]=q_upper[2];
                    q_upper[6]=-q_upper[2];
                    q_upper[12]=-q_upper[2];
                    ...
                    head_yaw=cal.move2pose(-yawRange+25*M_PI/180,double(count_upper*.005),0,4);
                    head_pitch=cal.move2pose(pitchRange-10*M_PI/180,double(count_upper*.005),0,4);
                    if(double(count_upper*.005)>=4){headFinished=true;}

                }
                else  if(scenario_hw=="m3"){
                    head_yaw=-yawRange+25*M_PI/180-cal.move2pose(-yawRange+25*M_PI/180,double(count_upper*.005),0,4);
                    head_pitch=pitchRange-10*M_PI/180-cal.move2pose(pitchRange-10*M_PI/180,double(count_upper*.005),0,4);
                    if(double(count_upper*.005)>=4){headFinished=true;}
                    ...
                    head_yaw =(yawRange-37*M_PI/180)*sin(double(count_upper*.005)/4*(2*M_PI))*(cal.move2pose(1,double(count_upper*.005),0,.5)-cal.move2pose(1,double(count_upper*.005),talkTime-.5,talkTime));
                    q_upper[2]=.03*sin(double(count_upper*.005)/4*(2*M_PI))*(cal.move2pose(1,double(count_upper*.005),0,.5)-cal.move2pose(1,double(count_upper*.005),talkTime-.5,talkTime));
                    q_upper[8]=q_upper[2];
                    q_upper[6]=-q_upper[2];
                    q_upper[12]=-q_upper[2];
                    ...
                    head_yaw =-cal.move2pose(yawRange*1.2,double(count_upper*.005),0,2);
                    if(double(count_upper*.005)>=2){headFinished=true;}
                }
                else  if(scenario_hw=="confirm"){
                    head_pitch =cal.move2pose(pitchRange-10*M_PI/180,double(count_upper*.005),0,1)-cal.move2pose(pitchRange-10*M_PI/180,double(count_upper*.005),1,2);
                    if(double(count_upper*.005)>=2){headFinished=true;}
                }
                else  if(scenario_hw=="t4"){
                    head_yaw =-yawRange+cal.move2pose(yawRange,double(count_upper*.005),0,2);
                    if(double(count_upper*.005)>=2){headFinished=true;}
                }

                else  if(scenario_hw=="respect"){
                    WaistPitch =cal.move2pose(WPitchRange-5*M_PI/180,double(count_upper*.005),0,2);
                    head_pitch =cal.move2pose(pitchRange-10*M_PI/180,double(count_upper*.005),0,2);
                    if(double(count_upper*.005)>=2){headFinished=true;}
                }
                else  if(scenario_hw=="c2"){
                    WaistPitch =(WPitchRange-5*M_PI/180)-cal.move2pose(WPitchRange-5*M_PI/180,double(count_upper*.005),0,2);
                    head_pitch =pitchRange-10*M_PI/180-cal.move2pose(pitchRange-10*M_PI/180,double(count_upper*.005),0,2);
                    if(double(count_upper*.005)>=2){headFinished=true;}
                }
                else if(scenario_hw=="wh(comeDown)"){

                    double dh=cal.move2pose(.02,double(count_upper*.005),t_r(0),t_r(2));
                    double l=.36+.37-dh;
                    ...
                    head_pitch=cal.move2pose(10*M_PI/180,double(count_upper*.005),t_r(0),t_r(1));
                    WaistPitch=cal.move2pose(10*M_PI/180,double(count_upper*.005),t_r(0),t_r(2));
                    ...
                    double dh=.02-cal.move2pose(.02,double(count_upper*.005),t_r(0),t_r(2));
                    double l=.36+.37-dh;
                    ...
                    q_upper[11]=q_upper[5];
                    //balloon=comment
                    // head_pitch=10*M_PI/180-cal.move2pose(10*M_PI/180,double(count*.005),t_r(0),t_r(1));
                    WaistPitch=10*M_PI/180-cal.move2pose(10*M_PI/180,double(count_upper*.005),t_r(0),t_r(2));
                    if(double(count_upper*.005)>=t_r(2)){headFinished=true;}

                }
                else if(scenario_hw=="wh(look)"){
                    head_roll=cal.move2pose(10*M_PI/180,double(count_upper*.005),0,1)-2*cal.move2pose(10*M_PI/180,double(count_upper*.005),2,5)+cal.move2pose(10*M_PI/180,double(count_upper*.005),6,7);
                    if(double(count_upper*.005)>=7){headFinished=true;}

                }

                else if(scenario_hw=="wh(release)"){
                    head_pitch=10*M_PI/180+cal.move2pose(-18*M_PI/180,double(count_upper*.005),0,3);
                    if(double(count_upper*.005)>=3){headFinished=true;}

                }

                else if(scenario_hw=="wh3_3"){
                    head_pitch=-6*M_PI/180-cal.move2pose(-6*M_PI/180,double(count_upper*.005),0,3);
                    if(double(count_upper*.005)>=3){headFinished=true;}
                    ...
                    double T_home=4;
                    head_pitch=cal.move2zero(head_pitch,t,T_home);
                    head_roll=cal.move2zero(head_roll,t,T_home);
                    head_yaw=cal.move2zero(head_yaw,t,T_home);
                    WaistPitch=cal.move2zero(WaistPitch,t,T_home);
                    WaistYaw=cal.move2zero(WaistYaw,t,T_home);
                    ...
                    WaistPitch=cal.move2pose(th1,t,0,4)+cal.move2pose(th2-th1,t,14,22)+cal.move2pose(-th2,t,32,36);
...
//            }
            ...
            //                PoseLFootOffline(2,0)=Trajectorymat[offlineTrajectoryIcontact](0,15)+cal.move2pose(OnlineTaskSpace._lenghtOfAnkle-Trajectorymat[offlineTrajectoryIcontact](0,15),Trajectorymat[num_Trajectorydata](0,0),Trajectorymat[offlineTrajectoryIcontact](0,0),offlineTrajectoryTend_ideal);
            //            }
            ...
            q_offlineData[5]=cal.saturate(SURENA_Offline.Links[5].JointAngle,-M_PI/.4,M_PI/4)+ankle_adaptation_switch*(left_first*teta_motor_R+(!left_first)*teta_motor_L);
            ...
            q_offlineData[11]=cal.saturate(SURENA_Offline.Links[11].JointAngle,-M_PI/.4,M_PI/4)+ankle_adaptation_switch*(left_first*teta_motor_L+(!left_first)*teta_motor_R);
...
//bool ImitationStart(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response){
//    if(!move_imitation_active){
//        move_imitation_active=true;
//        ROS_INFO("Imitation Started");
//        imit.imitationInit();
//        ros::Rate loop_rate(200);

//        while(move_imitation_active){
//            ros::spinOnce();

//            imit.imitationUpdate(imitationCode,simulation,a,b,c,d,e,f,g,h);
//            for (int i = 0; i < 34; ++i) {
//                q_offlineData[i]=imit.q[i];
//            }

//            if(imit.t_home>3){move_imitation_active=false;ROS_INFO("Imitation Ended");}
//            publish_data();
//            loop_rate.sleep();
...
bool GripingStart(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response){
    if(!move_griping_active){
        move_griping_active=true;
        ROS_INFO("Grip Started");
        grip.Init();

        ros::Rate loop_rate(200);

        while(move_griping_active){
            ros::spinOnce();

            grip.Updade(gripMsg,simulation);
            for (int i = 0; i < 34; ++i) {
                q_offlineData[i]=grip.q[i];
            }

           if(grip.time_deliver>=(grip.t_deliver(grip.t_deliver.cols()-1)+4)){move_griping_active=false; ROS_INFO("Grip Ended");}
            publish_data();
            loop_rate.sleep();
...
int main(int argc, char **argv)
{
    ...
    //ImitationService = nh.advertiseService("Imitation", ImitationStart);
    GripingService = nh.advertiseService("Grip", GripingStart);
    ...
    object_sub=nh.subscribe("/ai/object", 100, object_detect);

    sub = nh.subscribe("/surena/bump_sensor_state", 100, receiveFootSensor);
    //sub_basiction = nh.subscribe("/ai/basiction", 100, motion_detect);
...
ros::Rate loop_rate(200);
    ...
    //qDebug()<<;
ROS_INFO("start wholebodyDynamicControl3!");
