...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    ros::Publisher  trajectory_data_pub  = nh.advertise<std_msgs::Float32MultiArray>("trajectory_data_lower",100);
    ...
    while (ros::ok())
    {
        ...
        for (int i = 0; i <= 12; ++i) {
            trajectory_data.data.push_back(cntrl[i]);
        }
        trajectory_data.data.push_back(0);
        trajectory_data.data.push_back(0);
        trajectory_data.data.push_back(right_hand_pitch);
        for (int i = 1; i < 7; ++i) {
            trajectory_data.data.push_back(0);
        }
        trajectory_data.data.push_back(left_hand_pitch);
        for (int i = 1; i < 12; ++i) {
            trajectory_data.data.push_back(0);
        }
