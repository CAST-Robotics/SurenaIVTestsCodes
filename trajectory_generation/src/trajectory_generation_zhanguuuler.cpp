...
double RollModified_right=0;
double RollModified_left=0;
...
void left_right_switch_legs(){
   ...
   RollModified_left=RollModified_left+RollModified_right;
   RollModified_right=RollModified_left-RollModified_right;
   RollModified_left=RollModified_left-RollModified_right;
   RollModified_left=-RollModified_left;
   RollModified_right=-RollModified_right;
...
//     PoseRFoot(5,0)=0;
     ...
     if(scenario==1){
         ...
         double Ymax=.15;
         double Ypmax=.135;
         double pelvisRollmax=10;
         double pelvisPitchmax=13;
         double Zmax=.15;
         double ankleRollMax=15;
         double anklePitchMax=10;
         double Rollmodmax=4;
double ReferencePelvisHeight=0.91;
         double T_SP=3;//duration of startphase
         double T_p2s=3;//pelvis to side
         double T_a2s=6;//ankle to side
         double T_af=.4;//ankle fixed
         double T_a2h=8;//ankle2home
         double T_p2h=3;//pelvis2home
         double T_EPh=3;


         double TSCPR=T_SP+T_p2s-1.5;// start charge pelvis roll
         double TECPR=T_SP+T_p2s+2.5;
         double TSDCPR=T_SP+T_p2s+T_a2s+T_af+T_a2h-4.5;
         double TEDCPR=T_SP+T_p2s+T_a2s+T_af+T_a2h-1;

         double TSCRM=.9*(T_SP+T_p2s) ;// start charge  roll modify
         double TECRM=T_SP+T_p2s+T_a2s/2;
         double TSDCRM=T_SP+T_p2s+T_a2s+T_af+T_a2h;
         double TEDCRM=T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h;

         PoseRoot(2,0)=InitialPelvisHeight+cal.move2pose(ReferencePelvisHeight-InitialPelvisHeight,Time,0,T_SP)+
                 +cal.move2pose(-ReferencePelvisHeight+InitialPelvisHeight,Time,T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h,T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h+T_EPh);
         PoseRoot(1,0)=cal.move2pose(-Ypmax,Time,T_SP,T_SP+T_p2s)+cal.move2pose(Ypmax,Time,T_SP+T_p2s+T_a2s+T_af+T_a2h,T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h);
         PoseRoot(3,0)=cal.move2pose(pelvisRollmax,Time,TSCPR,TECPR)-cal.move2pose(pelvisRollmax,Time,TSDCPR,TEDCPR);

         PoseLFoot(1,0)=.115+cal.move2pose(Ymax,Time,T_SP+T_p2s,T_SP+T_p2s+T_a2s)
                 -cal.move2pose(Ymax,Time,T_SP+T_p2s+T_a2s+T_af,T_SP+T_p2s+T_a2s+T_af+T_a2h*.8);
         PoseLFoot(2,0)=lenghtOfAnkle+cal.move2pose(Zmax,Time,T_SP+T_p2s,T_SP+T_p2s+T_a2s)
                 -cal.move2pose(Zmax-.03,Time,T_SP+T_p2s+T_a2s+T_af,T_SP+T_p2s+T_a2s+T_af+T_a2h*.8)
                 -cal.move2pose(.03,Time,T_SP+T_p2s+T_a2s+T_af+T_a2h*.8,T_SP+T_p2s+T_a2s+T_af+T_a2h);
         PoseLFoot(3,0)=cal.move2pose(ankleRollMax,Time,T_SP+T_p2s,T_SP+T_p2s+T_a2s+1)
                 -cal.move2pose(ankleRollMax,Time,T_SP+T_p2s+T_a2s+T_af,T_SP+T_p2s+T_a2s+T_af+T_a2h*.8);


         RollModified_right=-cal.move2pose(Rollmodmax,Time,TSCRM,TECRM)+cal.move2pose(Rollmodmax,Time,TSDCRM,TEDCRM);

         q_ra(0)=q_rh(0)-cal.move2pose(1.5*q_rh(0),Time,0,T_SP)+cal.move2pose(1.5*q_rh(0),Time,T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h,T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h+T_EPh);
         q_la(0)=q_ra(0);
         q_lower[14]=cal.move2pose(cal.d2r(0),Time,0,T_SP)-cal.move2pose(cal.d2r(0),Time,T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h,T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h+T_EPh);
         q_la(1)=q_lh(1)+cal.move2pose(cal.d2r(60),Time,T_SP+T_p2s,T_SP+T_p2s+T_a2s)
                 -cal.move2pose(cal.d2r(60),Time,T_SP+T_p2s+T_a2s+T_af,T_SP+T_p2s+T_a2s+T_af+T_a2h*.8);

         //             T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h+T_EPh

         if(Time> T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h+T_EPh){break;}
     }

     else if(scenario==2){
         double Steplength=.23;
         double Ymax=.02;
         double Ypmax=.115;
         double pelvisRollmax=1*0;
         double pelvisPitchmax=0*20/2;
         double Zmax=.11;
         double ankleRollMax=1*0;
         double anklePitchMax=10/2;
         double Rollmodmax=4;
         double ReferencePelvisHeight=0.93;
         double T_SP=3;//duration of startphase
         double T_p2s=2;//pelvis to side
         double T_a2f=6;//ankle to front
         double T_af=.4;//ankle fixed
         double T_f2b=8;//ankle front to back
         double T_ab=.4;//ankle back fixed
         double T_ab2h=6;//ankle back to home
         double T_p2h=2;//pelvis2home
         double T_EPh=3;

         double motionTime=T_SP+T_p2s+T_a2f+T_af+T_f2b+T_ab+T_ab2h+T_p2h+T_EPh;


         double TSCPR=T_SP+T_p2s+T_a2f+T_af;// start charge pelvis roll
         double TECPR=T_SP+T_p2s+T_a2f+T_af+T_f2b/2;
         double TSDCPR=T_SP+T_p2s+T_a2f+T_af+T_f2b/2;
         double TEDCPR=T_SP+T_p2s+T_a2f+T_af+T_f2b;

         double TSCPP=T_SP+T_p2s+.2*T_a2f;// start charge pelvis pitch
         double TECPP=T_SP+T_p2s+.9*T_a2f;
         double TSMPP=T_SP+T_p2s+T_a2f+T_af+.1*T_f2b;
         double TEMPP=T_SP+T_p2s+T_a2f+T_af+.9*T_f2b;
         double TSDCPP=T_SP+T_p2s+T_a2f+T_af+T_f2b+T_ab+.1*T_ab2h;
         double TEDCPP=T_SP+T_p2s+T_a2f+T_af+T_f2b+T_ab+.9*T_ab2h;

         double TSCRM=.9*(T_SP+T_p2s) ;// start charge  roll modify
         double TECRM=T_SP+T_p2s+T_a2f/2;
         double TSDCRM=motionTime-T_EPh-T_p2h;
         double TEDCRM=motionTime-T_EPh;

         PoseRoot(2,0)=InitialPelvisHeight+cal.move2pose(ReferencePelvisHeight-InitialPelvisHeight,Time,0,T_SP)+
                 +cal.move2pose(-ReferencePelvisHeight+InitialPelvisHeight,Time,motionTime-T_EPh,motionTime);

         PoseRoot(1,0)=cal.move2pose(-Ypmax,Time,T_SP,T_SP+T_p2s)
                 +cal.move2pose(Ypmax,Time,motionTime-T_EPh-T_p2h,motionTime-T_EPh);

         PoseRoot(3,0)=cal.move2pose(pelvisRollmax,Time,TSCPR,TECPR)-cal.move2pose(pelvisRollmax,Time,TSDCPR,TEDCPR);

         PoseRoot(4,0)=cal.move2pose(-pelvisPitchmax,Time,TSCPP,TECPP)+cal.move2pose(2*pelvisPitchmax,Time,TSMPP,TEMPP)
                 +cal.move2pose(-pelvisPitchmax,Time,TSDCPP,TEDCPP);

         PoseLFoot(0,0)=cal.move2pose(Steplength,Time,T_SP+T_p2s,T_SP+T_p2s+T_a2f)
                 +cal.move2pose(-2*Steplength,Time,T_SP+T_p2s+T_a2f+T_af,T_SP+T_p2s+T_a2f+T_af+T_f2b)
                 +cal.move2pose(Steplength,Time,motionTime-T_EPh-T_p2h-T_ab2h,motionTime-T_EPh-T_p2h);

         PoseLFoot(1,0)=.115+cal.move2pose(Ymax,Time,T_SP+T_p2s+T_a2f+T_af,T_SP+T_p2s+T_a2f+T_af+.5*T_f2b)
                 -cal.move2pose(Ymax,Time,T_SP+T_p2s+T_a2f+T_af+.5*T_f2b,T_SP+T_p2s+T_a2f+T_af+T_f2b);

         PoseLFoot(2,0)=lenghtOfAnkle+cal.move2pose(Zmax,Time,T_SP+T_p2s,T_SP+T_p2s+T_a2f)
                 -cal.move2pose(Zmax-.03,Time,T_SP+T_p2s+T_a2f+T_af+T_f2b+T_ab,T_SP+T_p2s+T_a2f+T_af+T_f2b+T_ab+.8*T_ab2h)
                 -cal.move2pose(.03,Time,T_SP+T_p2s+T_a2f+T_af+T_f2b+T_ab+.8*T_ab2h,T_SP+T_p2s+T_a2f+T_af+T_f2b+T_ab+T_ab2h);

         PoseLFoot(3,0)=cal.move2pose(ankleRollMax,Time,TSCPR,TECPR)-cal.move2pose(ankleRollMax,Time,TSDCPR,TEDCPR);

         PoseLFoot(4,0)=cal.move2pose(-anklePitchMax,Time,TSCPP,TECPP)+cal.move2pose(2*anklePitchMax,Time,TSMPP,TEMPP)
                 +cal.move2pose(-anklePitchMax,Time,TSDCPP,TEDCPP);


         RollModified_right=-cal.move2pose(Rollmodmax,Time,TSCRM,TECRM)+cal.move2pose(Rollmodmax,Time,TSDCRM,TEDCRM);

         q_ra(0)=q_rh(0)-cal.move2pose(2.5*q_rh(0),Time,0,T_SP)+cal.move2pose(2.5*q_rh(0),Time,motionTime-T_EPh,motionTime);
         q_la(0)=q_ra(0);
         q_lower[14]=cal.move2pose(cal.d2r(10),Time,0,T_SP)-cal.move2pose(cal.d2r(10),Time,motionTime-T_EPh,motionTime);
//         q_la(1)=q_lh(1)+cal.move2pose(cal.d2r(60),Time,T_SP+T_p2s,T_SP+T_p2s+T_a2s)
//                 -cal.move2pose(cal.d2r(60),Time,T_SP+T_p2s+T_a2s+T_af,T_SP+T_p2s+T_a2s+T_af+T_a2h*.8);

         //             T_SP+T_p2s+T_a2s+T_af+T_a2h+T_p2h+T_EPh

         if(Time> motionTime){break;}
     }
     ...
     if(fabs(ankladpt.AnkleZL_input-ankladpt.AnkleZL_output)>1e-6){qDebug()<<"z_in_l="<<ankladpt.AnkleZL_input<<"  z_out_l="<<ankladpt.AnkleZL_output;}
     if(fabs(ankladpt.AnkleZR_input-ankladpt.AnkleZR_output)>1e-6){qDebug()<<"z_in_r="<<ankladpt.AnkleZR_input<<"  z_out_r="<<ankladpt.AnkleZR_output;}
...
//cal.numplot(-PoseRFoot(1,0),.115,.115+Ymax);
     ...
     q_lower[2]=SURENA.Links[2].JointAngle+(!simulation)*RollModified_right*M_PI/180;
     ...
     q_lower[8]=SURENA.Links[8].JointAngle+(!simulation)*RollModified_left*M_PI/180;
