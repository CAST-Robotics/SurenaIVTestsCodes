...
QByteArray alai3;
QByteArray alai4;
QByteArray AnkleRight;
QByteArray AnkleLeft;
QByteArray AnkleRightInc;
QByteArray AnkleLeftInc;
QByteArray AnkleRightSat;
QByteArray AnkleLeftSat;
double outControl=0;
double CheckPoint1;
double CheckPoint2;
...
void qc_initial(const sensor_msgs::JointState & msg){
    if (qc_initial_bool){
        ...
        if(move_active_offlineData||move_active_offlineTrajectory||move_lower_active||move_upper_activate){
...
void incData(const sensor_msgs::JointState & msg){
    for (int i = 0; i <= 31; ++i) {
        incMotors[i]=int(msg.position[i+1]);
    }
...
bool StartWalk(trajectory_generation::walkRequest &req,trajectory_generation::walkResponse &res)
{
    ...
    if(!move_lower_active){
        ...
        DurationOfStartPhase=fabs(OnlineTaskSpace.ReferencePelvisHeight-PelvisCurrentHeight)*3/.05+0.0;
        leftzstop=false;
        rightzstop=false;
        GlobalTime=0;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber incSubscriber = nh.subscribe("/surena/inc_joint_state", 1000, incData);
    ros::ServiceServer WalkService = nh.advertiseService("Walk", StartWalk);
    ros::ServiceServer StopService = nh.advertiseService("StopWalk", StopWalk);
    ros::ServiceServer HandMoveService = nh.advertiseService("HandMove", handMove);
    ros::ServiceServer StopTalkService = nh.advertiseService("StopTalk", StopTalk);
    ros::ServiceServer offlineDataService = nh.advertiseService("offlineData", offlineData_start);
    ros::ServiceServer offlineTrajectoryService = nh.advertiseService("offlineTrajectory", offlineTrajectory_start);
    ...
    MatrixXd R_right_palm(3,3); MatrixXd R_left_palm(3,3);
    ...
    while (ros::ok())
    {
        ...
        if(move_lower_active){
            ...
            if ( GlobalTime<DurationOfStartPhase) {
                ...
                qDebug()<<"OffsetPitch"<<pelvisController.OffsetPitch;
            }
            //       if(GlobalTime>DurationOfStartPhase+OnlineTaskSpace.TGait+OnlineTaskSpace.TDs+OnlineTaskSpace.TLastSS){
            if(GlobalTime>DurationOfStartPhase){
                ...
                //outControl = move2pose(1*M_PI/180,GlobalTime,DurationOfStartPhase,5+DurationOfStartPhase);
                ...
                //Pelvis Controller 2rosr
                //pelvisController.SudoTorqueController(pelvis_angularVelocity[1],pelvis_orientation_pitch,20,0.005,incMotors[1],incMotors[4],qc_offset[1],qc_offset[4]);
                pelvisController.SudoTorqueController2(pelvis_angularVelocity[1],pelvis_orientation_pitch,100,0.005,incMotors[1],incMotors[4],qc_offset[1],qc_offset[4]);
                //qDebug()<<"Right"<<incMotors[1]<<"and Left"<<incMotors[4];
//                qDebug()<<"Inverse 1"<<QC.ankle_pitch_reverse(QC.ankle_pitch(1*M_PI/180,0.034),0.034)*180/M_PI;
//                qDebug()<<"Inverse -1"<<QC.ankle_pitch_reverse(QC.ankle_pitch(-1*M_PI/180,0.034),0.034)*180/M_PI;
//                qDebug()<<"Inverse 10"<<QC.ankle_pitch_reverse(QC.ankle_pitch(10*M_PI/180,0.034),0.034)*180/M_PI;
//                qDebug()<<"Inverse -15"<<QC.ankle_pitch_reverse(QC.ankle_pitch(-15*M_PI/180,0.034),0.034)*180/M_PI;
                alai2.append(QString::number(checkalai)+"\n");
                alai3.append(QString::number(pelori)+"\n");
                alai4.append(QString::number(pelav)+"\n");
                AnkleRight.append(QString::number(pelvisController.RightSudoTorqueControllerOutput)+"\n");
                AnkleLeft.append(QString::number(pelvisController.LeftSudoTorqueControllerOutput)+"\n");
                AnkleLeftInc.append(QString::number(incMotors[4]-qc_offset[4])+"\n");
                AnkleRightInc.append(QString::number(incMotors[1]-qc_offset[1])+"\n");
            ...
            if (GlobalTime>DurationOfStartPhase && GlobalTime<(DurationOfStartPhase+OnlineTaskSpace.MotionTime)){
                ...
                //                if(GlobalTime>DurationOfStartPhase+OnlineTaskSpace.MotionTime)
                //Pelvis Vibration Controller
                //                                outControl = pelvisController.CPController(pelvis_angularVelocity[1],pelvis_orientation_pitch,PelvisVibControlllerCoef,0.005);
                //                                double checkalai = pelvis_orientation_pitch + 18*pelvis_angularVelocity[1]/M_PI;
                //                                double pelori = pelvis_orientation_pitch;
                ...
                if (OnlineTaskSpace.globalTime<=OnlineTaskSpace.MotionTime){
                    ...
                    if(turning&&TurningRadius>=.2){alpha=.95;}
                    ...
                    if(sidewalk){
                        ...
                        // if(fmod(GlobalTime,.5)<.006){ qDebug()<<GlobalTime<<"\t"<<PoseRoot(1)<<"\t"<<PoseRFoot(1)<<"\t"<<PoseLFoot(1)<<endl;}
            ...
            if(GlobalTime>=DurationOfendPhase+DurationOfStartPhase+OnlineTaskSpace.MotionTime||(JustStartphase&&GlobalTime>=DurationOfStartPhase)){
                PelvisCurrentHeight=PoseRoot(2);
...
//            q_lower[5]=saturate(pelvisController.RightSudoTorqueControllerOutput,-M_PI*2/180,M_PI*2/180);
//            q_lower[11]=saturate(pelvisController.LeftSudoTorqueControllerOutput,-M_PI*2/180,M_PI*2/180);
//            AnkleRightSat.append(QString::number(q_lower[5])+"\n");
//            AnkleLeftSat.append(QString::number(q_lower[11])+"\n");
//            q_lower[5] = 0;
//            q_lower[11] = 0;
//            q_lower[5]=saturate(outControl,-M_PI*2/180,M_PI*2/180);
//            q_lower[11]=saturate(outControl,-M_PI*2/180,M_PI*2/180);
            //QCgenerator checkpoint;
            //qDebug()<<"check"<<checkpoint.ankle_pitch(-3*M_PI/180,0.034)*180/M_PI;
        ...
        if(move_upper_activate){
            ...
            if(initializing){
                ...
                if(scenario_l=="lookAtHorizon"){fingers_mode_l=10;}
                ...
                if(scenario_hw=="face"){
                    ...
                    head_pitch=move2pose(pitchRange,double(count_upper*.005),0,2);
                    if(double(count_upper*.005)>=2){headFinished=true;}
                ...
                //look at hands
                    ...
                    head_yaw=move2pose(yawRange-20*M_PI/180,double(count_upper*.005),0,2);
                    head_pitch=pitchRange-move2pose(pitchRange,double(count_upper*.005),0,2);
                    WaistYaw=move2pose(WYawRange,double(count_upper*.005),2,4);
                    ...
                    double dh=.02-move2pose(.02,double(count_upper*.005),t_r(0),t_r(2));
                    double l=.36+.37-dh;
        ...
        if(move_active_offlineTrajectory){
            ...
            //            }
            //            RollModified_offline=Trajectorymat[num_Trajectorydata](0,19);

            //            //if(num_Trajectorydata%20==0){qDebug()<<e<<"\t"<<f<<"\t"<<g<<"\t"<<h;}
            //            if(Trajectorymat[num_Trajectorydata](0,15)>OnlineTaskSpace._lenghtOfAnkle+.12){offlineTrajectoryContact=true;
            //                qDebug()<<"toosh";
            //            }
            //            if (offlineTrajectoryContact&&(a>bump_threshold||b>bump_threshold||c>bump_threshold||d>bump_threshold)){
            //                offlineTrajectoryContact=false;
            ...
            //            }
            //            if(!offlineTrajectoryContact&&(Trajectorymat[num_Trajectorydata](0,0)>Trajectorymat[offlineTrajectoryIcontact-1](0,0))){
            ...
            //                        // if(num_Trajectorydata%20==0){ qDebug()<<Trajectorymat[num_Trajectorydata](0,0)<<"\t"<<Trajectorymat[offlineTrajectoryIcontact](0,15)+move2pose(OnlineTaskSpace._lenghtOfAnkle-Trajectorymat[offlineTrajectoryIcontact](0,15),Trajectorymat[num_Trajectorydata](0,0),Trajectorymat[offlineTrajectoryIcontact](0,0),offlineTrajectoryTend_ideal);}
            ...
            //            R_P_Offline<<cos(PoseRootOffline(4)*M_PI/180),0,sin(PoseRootOffline(4)*M_PI/180),
            //                    0,1,0,
            //                    -sin(PoseRootOffline(4)*M_PI/180),0,cos(PoseRootOffline(4)*M_PI/180);

            //            R_F_R_Offline<<cos(PoseRFootOffline(4)*M_PI/180),0,sin(PoseRFootOffline(3)*M_PI/180),
            //                    0,1,0,
            //                    -sin(PoseRFootOffline(4)*M_PI/180),0,cos(PoseRFootOffline(4)*M_PI/180);
            //            R_F_L_Offline<<cos(PoseLFootOffline(4)*M_PI/180),0,sin(PoseLFootOffline(4)*M_PI/180),
            //                    0,1,0,
            ...
            //            q_offlineData[2]=SURENA_Offline.Links[2].JointAngle;//+RollModified_offline*M_PI/180;
            //            q_offlineData[3]=SURENA_Offline.Links[3].JointAngle;
            //            q_offlineData[4]=SURENA_Offline.Links[4].JointAngle;
            //            q_offlineData[5]=SURENA_Offline.Links[5].JointAngle;//+ankle_adaptation_switch*(left_first*teta_motor_R+(!left_first)*teta_motor_L);
            ...
            //            q_offlineData[8]=SURENA_Offline.Links[8].JointAngle;
            //            q_offlineData[9]=SURENA_Offline.Links[9].JointAngle;
            //            q_offlineData[10]=SURENA_Offline.Links[10].JointAngle;
            //            q_offlineData[11]=SURENA_Offline.Links[11].JointAngle;//+ankle_adaptation_switch*(left_first*teta_motor_L+(!left_first)*teta_motor_R);
            ...
            double t_offline;
            double RollModified_offline_left;double RollModified_offline_right;
            MatrixXd PoseRootOffline(6,1);//position of pelvis respected to global coordinate
            MatrixXd PoseRFootOffline(6,1);//position of right ankle joint respected to global coordinate
            MatrixXd PoseLFootOffline(6,1);//position of left ankle joint respected to global coordinate
            MatrixXd R_P_Offline(3,3);  R_P_Offline=MatrixXd::Identity(3,3);
        ...
        if(move_upper_activate||move_lower_active||move_active_offlineData||move_active_offlineTrajectory){
            ...
            q[5]=saturate(q[5],d2r(-36),d2r(45));
            q[11]=saturate(q[11],d2r(-36),d2r(45));
    ...
    //QFile alaifile1("/home/cast/Desktop/TestController/ControllerCheckCO.txt");
    QFile alaifile2("/home/cast/Desktop/TestController/ControllerCheckCP.txt");
    QFile alaifile3("/home/cast/Desktop/TestController/ControllerCheckOri.txt");
    QFile alaifile4("/home/cast/Desktop/TestController/ControllerCheckAV.txt");
    QFile AnkleRightFile("/home/cast/Desktop/TestController/rightAnkle.txt");
    QFile AnkleRightSatFile("/home/cast/Desktop/TestController/rightAnkleSaturated.txt");
    QFile AnkleLeftSatFile("/home/cast/Desktop/TestController/leftAnkleSaturated.txt");
    QFile AnkleLeftFile("/home/cast/Desktop/TestController/lefttAnkle.txt");
    QFile AnkleRightIncFile("/home/cast/Desktop/TestController/rightAnkleInc.txt");
    QFile AnkleLeftIncFile("/home/cast/Desktop/TestController/leftAnkleInc.txt");
//    alaifile1.remove();
//    alaifile1.open(QFile::ReadWrite);
//    alaifile1.write(alai1);
//    alaifile1.close();


    alaifile2.remove();
    alaifile2.open(QFile::ReadWrite);
    alaifile2.write(alai2);
    alaifile2.close();
    alaifile3.remove();
    alaifile3.open(QFile::ReadWrite);
    alaifile3.write(alai3);
    alaifile3.close();
    alaifile4.remove();
    alaifile4.open(QFile::ReadWrite);
    alaifile4.write(alai4);
    alaifile4.close();
    AnkleRightFile.remove();
    AnkleRightFile.open(QFile::ReadWrite);
    AnkleRightFile.write(AnkleRight);
    AnkleRightFile.close();
    AnkleLeftFile.remove();
    AnkleLeftFile.open(QFile::ReadWrite);
    AnkleLeftFile.write(AnkleLeft);
    AnkleLeftFile.close();

    AnkleRightIncFile.remove();
    AnkleRightIncFile.open(QFile::ReadWrite);
    AnkleRightIncFile.write(AnkleRightInc);
    AnkleRightIncFile.close();
    AnkleLeftIncFile.remove();
    AnkleLeftIncFile.open(QFile::ReadWrite);
    AnkleLeftIncFile.write(AnkleLeftInc);
    AnkleLeftIncFile.close();

    AnkleRightSatFile.remove();
    AnkleRightSatFile.open(QFile::ReadWrite);
    AnkleRightSatFile.write(AnkleRightSat);
    AnkleRightSatFile.close();
    AnkleLeftSatFile.remove();
    AnkleLeftSatFile.open(QFile::ReadWrite);
    AnkleLeftSatFile.write(AnkleLeftSat);
    AnkleLeftSatFile.close();
