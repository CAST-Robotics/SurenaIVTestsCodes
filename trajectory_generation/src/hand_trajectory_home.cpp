...
void abs_read(const sensor_msgs::JointState & msg){
    if (abs_initial_bool){
        ...
        absolute_q0_r(1)=double(absolute_sensor_r(1)-(-889+2275+50))/8192*2*M_PI;
...
//        absolute_q0_l(2)=-double(absolute_sensor_l(2)+344)/8192*2*M_PI;
        absolute_q0_l(2)=-double(absolute_sensor_l(2)+510)/8192*2*M_PI;
        absolute_q0_l(3)=-double(absolute_sensor_l(3)-153)/8192*2*M_PI;
        qDebug()<<"absolutes:";
...
q_la.fill(0);
    ...
    qr_home<<13*M_PI/180,-10*M_PI/180,0,-25*M_PI/180, 0,0,0;
    ql_home<<13*M_PI/180, 10*M_PI/180,0,-25*M_PI/180, 0,0,0;

//    qr_home<<0,-10*M_PI/180,0,0, 0,0,0;
//    ql_home<<0, 10*M_PI/180,0,0, 0,0,0;
...
//        q_motor_r[0]=qra_offset[0];
//        q_motor_r[1]=qra_offset[1];
//        q_motor_r[2]=qra_offset[2];
//        q_motor_r[3]=qra_offset[3];
//        q_motor_l[0]=qla_offset[0];
//        q_motor_l[1]=qla_offset[1];
//        q_motor_l[2]=qla_offset[2]+int(move2pose(5,time,0,1));
//        q_motor_l[3]=qla_offset[3];
        ...
        ros::spinOnce();
        loop_rate.sleep();
        if(time>3){qDebug()<<q_motor_l[2];break;}
